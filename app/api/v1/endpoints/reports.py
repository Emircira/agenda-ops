"""REST: durum raporu (SITREP) — HTML (yazdır) veya JSON."""
...
from app.repositories.deps import get_alert_repository, get_macro_repository
...
async def get_daily_sitrep(
    ...
    """
    Günlük durum özeti (SITREP).

    - ``format=html`` (varsayılan): yazdırmaya uygun tek sayfa HTML.
    - ``format=json``: ham veri paketi (API / otomasyon).
    """
    report = await generate_daily_sitrep(macro_repo, alert_repo)
